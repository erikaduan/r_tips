---
title: "Automating R Markdown report generation (Part 2)"  
author: "Erika Duan"
date: "`r Sys.Date()`"
output:
  github_document:  
...
This post follows from [an earlier post](https://github.com/erikaduan/R-tips/blob/master/03_blog_posts/2020-08-30_automating_RMDs/2020-08-30_automating_RMDs.md) describing the preliminary steps towards automated reporting in R.    

Creating an automated reporting workflow requires the following setup:    

1. A consistent file structure for project inputs (i.e. raw data), outputs (i.e. clean data) and analysis reports.  
2. A data sourcing and cleaning script that can be refreshed.    
3. An R Markdown template report that references report variables as yaml parameters.     
4. A report automation script.    

```{r, echo = FALSE, fig.align = 'center', fig.show = 'hold', out.width = '60%'}     
knitr::include_graphics("../../02_figures/2020-09-10_automated-reporting-file-setup.png")    
```
...
1. Downloads raw data from its original source (i.e. an internet hyperlink or data API).  
2. Cleans raw data.    
3. Saves the clean dataset as an output.    

This setup allows us to automate reporting changes whenever the original source data is updated, assuming that there are no changes to data formating.         
...
The R Markdown template report contains the code for data visualisation and reporting outcomes.   

```{r, echo = FALSE, fig.align = 'center', fig.show = 'hold', out.width = '100%'}     
knitr::include_graphics("../../02_figures/2020-09-10_automated-reporting-template-RMD.png")    
```

The key difference between a standard data analytical report and an R Markdown template report is the absence of hardcoded variables and exploratory code in the template report. The template report should contain the minimal and most efficient code required to generate your report outputs (i.e. figures, tables and summary text).   

An example of a chunk of code found inside my R Markdown template report is shown below. The only step that uses hard coded variables is the position of annotated text on the plot (this position remains fixed in the report).    
...
**Note:** As you are creating your template report, you can knit it into an html report and test it iteratively until you generate final polished product.       
...
2. The function `render` which uses your R Markdown template report as its input file and your list of variables as the parameter of interest, to generate a series of output files.     
...
Violà! You can now enjoy the products of your automated reporting workflow.    

```{r, echo = FALSE, fig.align = 'center', fig.show = 'hold', out.width = '100%'}     
knitr::include_graphics("../../02_figures/2020-09-10_automated-reporting-final-product.png")    
```
