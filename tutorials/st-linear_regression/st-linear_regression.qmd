...
The simplest linear regression model is a line of best fit through the 2D Cartesian plane. To construct this model, we only need to estimate the two parameters of an unknown straight line:  

+ The y-intercept, which we refer to as $\beta_0$.  
+ The slope, which we refer to as $\beta_1$   

We assume that there is a **true model** that precisely predicts our outcome of interest $Y_i$ based on our independent variable of interest $X_i$. The true model has the form $Y_i = \beta_0 + \beta_1X_1 + \epsilon_i$, where $\epsilon_i$ represents error due to natural variation, because objects do not behave like perfect clones of each other in the real world.    

```{r, echo=FALSE, results='markup', fig.align='center', out.width='80%'} 
knitr::include_graphics("../../figures/st-linear_regression-true_model.gif")      
``` 

Because there is always error due to natural variation, we view each observation of $Y_i$ as being drawn from a normal distribution of many possible values. By making some assumptions about $\epsilon_i$, we can claim that the **mean** of the probability distribution of $Y_i$ is $E(Y_i) = \beta_0 + \beta_1X_1$. This is the unknown straight line that we want to estimate.      

```{r, echo=FALSE, results='markup', fig.align='center', out.width='80%'} 
knitr::include_graphics("../../figures/st-linear_regression-simple_model_structure.gif")      
``` 

We want to use our training data set to find the best estimates of $\beta_0$ and $\beta_1$. This means finding the line that travels closest through all the training data set observations. This line represents our **best estimated model**, which has the form $\hat Y_i = b_0 + b_1X_i$. It has a y-intercept of $b_0$ and slope of $b_1$. 

The simple linear regression model coefficients $b_0$ and $b_1$ are therefore our best estimates of the unknown $\beta_0$ and $\beta_1$. The values for $b_0$ and $b_1$ may be different depending on which observations are included in our training data set.        
...
In mathematical terms, we think our true unknown model and best estimated model have the following forms.    
