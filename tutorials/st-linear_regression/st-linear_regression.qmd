...
The model coefficients $b_0, \cdots, b_4$ directly contribute to the interpretation of how our model predicts mean monthly pet influencer income.  

![](../../figures/st-linear_regression-coefficients_explained_1.svg)  

![](../../figures/st-linear_regression-coefficients_explained_2.svg)  

Let us examine our model coefficients. We can output them into a tabular format using the `tidy()` function from the [`broom`](https://cran.r-project.org/web/packages/broom/vignettes/broom.html) package.      
...
|:-------|:------------|   
| Residual mean square (MSE) | $\frac{\sum_{i=1}^n (Y_i - \hat {Y_i})^2}{n-2}$ | A metric to evaluate the magnitude of the error between the true and estimated outcome. In linear regression, MSE is specifically the point estimator for $Var(\epsilon_i)$ assuming that $\epsilon_i$ is normally distributed. The MSE should be evaluated on the test data set. |       
| Residual standard error (RSE) | $\sqrt{\frac{\sum_{i=1}^n (Y_i - \hat {Y_i})^2}{n-2}}$ | Another metric to evaluate the magnitude of the error between the true and estimated outcome. RSE can be preferred over MSE as it has the same units as the outcome $Y_i$. The RSE should be evaluated on the test data set. |      
| F statistic |   | A statistical test with the null hypothesis that ... A metric describing the properties of the trained model. |  
| Multiple $r^2$ | A metric describing the properties of the trained model.  |  ... |    
| Adjusted $r^2$ | A metric describing the properties of the trained model.  |  ... |   
