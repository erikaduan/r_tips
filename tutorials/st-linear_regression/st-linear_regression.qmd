...
![](../../figures/st-linear_regression-y_normal_distribution.svg){fig-width=3}
...
The model coefficients $b_0$ and $b_1$ are our best point estimates of the unknown $\beta_0$ and $\beta_1$ parameters. The values for $b_0$ and $b_1$ are usually slightly different depending on which observations are present in our training data set.        
...
Mathematically, our best estimated model has the following form, where $b_0, \cdots, b_4$ are the point estimates for $\beta_0, \cdots, \beta_4$ respectively.    

![](../../figures/st-linear_regression-tutorial_model_structure.svg)

Let us examine the values for $b_0, \cdots, b_4$ (also known as the coefficients) of our model. We can output them into a tabular format using the `tidy()` function from the [`broom`](https://cran.r-project.org/web/packages/broom/vignettes/broom.html) package.      
...
After examining the **point estimates**, **standard errors** and **p-values** of our model coefficients, we can claim that the following associations exist, provided that our modelling assumptions are reasonable:   
...
The standard error and p-value are actually more important than the precise value of the point estimate. The standard error estimates the range of values that our true model parameters may fall within. The p-value is used as a yardstick to conclude if our true model parameter is non-zero.    

We can output and plot the 95% confidence intervals for our model coefficients to aid the interpretation of our model.  
...
#TODO 
But how do we know if a model is good enough? We need to examine the model evaluation metrics.  
