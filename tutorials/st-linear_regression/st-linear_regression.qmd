...
| Residual standard error (RSE) | $\sqrt{\frac{\sum_{i=1}^n (Y_i - \hat {Y_i})^2}{n-p}}$ | Another metric to evaluate the magnitude of the error between the true and estimated outcome. RSE can be preferred over MSE as it has the same units as the outcome $Y_i$. The RSE should be evaluated on the test data set. |      
| F statistic | $\frac{MSR}{MSE} \sim F(p-1, n-p)$ where $MSR = \frac{\sum_{i=1}^n (\hat{Y_i} - \bar Y)^2}{p-1}$ | A statistical test with a null hypothesis that all model coefficients are zero. This metric describes a statistical property of our trained model and is not really useful for model evaluation or even interpretation. |    
